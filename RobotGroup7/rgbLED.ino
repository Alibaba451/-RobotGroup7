...
const int ExpectedRed[3] = { 28, 6, 9 };
const int ExpectedGreen[3] = { 18, 28, 23 };
const int ExpectedBlue[3] = { 6, 9, 19 };
const int ExpectedYellow[3] = { 50, 34, 20 };
...
void checkColor() {
  ...
  if (abs(avgRed - ExpectedRed[0] <= threshold) && abs(avgGreen - ExpectedRed[1] <= threshold) && abs(avgBlue - ExpectedRed[2] <= threshold && avgRed > avgGreen)) {
    ...
    if (abs(avgRed - ExpectedBlue[0] <= threshold) && abs(avgGreen - ExpectedBlue[1] <= threshold) && abs(avgBlue - ExpectedBlue[2] <= threshold)) {
